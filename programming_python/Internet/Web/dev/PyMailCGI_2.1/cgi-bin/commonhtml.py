...
def pageheader(app='PyMailCGI', color='#FFFFFF', kind='main', info=''):
    ...
    print
    '<html><head><title>%s: %s page (PP3E)</title></head>' % (app, kind)
    print
    '<body bgcolor="%s"><h1>%s %s</h1><hr>' % (color, app, (info or kind))
...
def pagefooter(root='pymailcgi.html'):
    ...
    print
    '<a href="../%s">Back to root page</a>' % root
    print
    '</body></html>'
...
def pagelisttable(linklist):  # show list in a table
    print
    '<p><table border>'  # escape text to be safe
    count = 1
    for (text, cgiurl, parmdict) in linklist:
        link = formatlink(cgiurl, parmdict)
        text = cgi.escape(text)
        print
        '<tr><th><a href="%s">View</a> %d<td>\n %s' % (link, count, text)
        count = count + 1
    print
    '</table>'
...
def messagearea(headers, text, extra=''):
    ...
    for hdr in ('From', 'To', 'Cc', 'Subject'):
        ...
        print
        '<tr><th align=right>%s:' % hdr
        print
        '    <td><input type=text '
        print
        '    name=%s value="%s" %s size=60>' % (hdr, val, extra)
    print
    '<tr><th align=right>Text:'
    print
    '<td><textarea name=text cols=80 rows=10 %s>' % extra
    print
...
def viewattachmentlinks(partnames):
    ...
    """
    print
    '<hr><table border cellpadding=3><tr><th>Parts:'
    for filename in partnames:
        basename = os.path.basename(filename)
        filename = filename.replace('\\', '/')  # windows hack
        print
        '<td><a href=../%s>%s</a>' % (filename, basename)
    print
    '</table><hr>'
...
def viewpage(msgnum, headers, text, form, hdrstext, parts=[]):
    ...
    print
    '<form method=get action="%sonViewPageAction.py">' % urlroot
    print
    '<input type=hidden name=mnum value="%s">' % msgnum
    print
    '<input type=hidden name=user value="%s">' % user  # from page|url
    print
    '<input type=hidden name=site value="%s">' % site  # for deletes
    print
    '<input type=hidden name=pswd value="%s">' % pswd  # pswd encoded
    messagearea(headers, text, 'readonly')
    if parts: viewattachmentlinks(parts)

    # onViewPageAction.quotetext needs date passed in page
    print
    '<input type=hidden name=Date value="%s">' % headers.get('Date', '?')

    # EXPERIMENTAL
    # delete needs hdrs text for inbox synch tests: can be multi-K large
    hdrstext = cgi.escape(hdrstext, quote=True)  # escape '"' too
    print
    '<input type=hidden name=Hdrstext value="%s">' % hdrstext
    ...
    print
    '</table></form>'  # no 'reset' needed here
    pagefooter()
...
def sendattachmentwidgets(maxattach=3):
    ...
    for i in range(1, maxattach + 1):
        print
        '<input size=80 type=file name=attach%d><br>' % i
    print
    '</p>'
...
def editpage(kind, headers={}, text=''):
    ...
    print
    'action="%sonEditPageSend.py">' % urlroot
...
def errorpage(message, stacktrace=True):
    ...
    print
    '<h2>Error Description</h2><p>', message
    print
    '<h2>Python Exception</h2><p>', cgi.escape(str(exc_type))
    print
    '<h2>Exception details</h2><p>', cgi.escape(str(exc_value))
    if stacktrace:
        print
        '<h2>Exception traceback</h2><p><pre>'
        import traceback
...
def confirmationpage(kind):
    ...
    print
    '<h2>%s operation was successful</h2>' % kind
    print
