...
def generateRecord(record):
    ...
    for field in record.keys():
        if record[field] != '' and field != 'Description':
            print
            rowhtml % (field, cgi.escape(str(record[field])))
    ...
    print
    '<p><b>%s</b><br><pre>%s</pre><hr>' % (field, cgi.escape(text))
...
def generateIndexOnly(dbase, sortkey, kind):
    ...
    print
    '<h2>Index</h2><ul>'  # for load on click
    for key in keys:
...
def generateIndexed(dbase, sortkey):
    ...
    for key in keys:
        ...
        print
        html % (section, cgi.escape(str(key)) or '?')
        section = section + 1
    print
    '</ul><hr>'
    ...
    for key in keys:
        ...
        print
        html % (section, cgi.escape(str(key)))
...
def generatePage(dbase, kind='Errata'):
    ...
    print
    '<title>PP2E %s list</title>' % kind
    print
    '<h1>%s list, sorted by "%s"</h1><hr>' % (kind, str(sortkey))
