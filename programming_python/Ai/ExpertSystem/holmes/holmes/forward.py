...
def forward(kbase, facts, *pmode):
    ...
    except stop_chaining:
        pass  # user can stop it
...
def ask_user(fact, known, why):
    ...
    while 1:
        ...
        if ans in ['y', 'Y', 'yes', 'YES']:
            ...
            print
            'to see if rule', why, 'can be applied'
        elif ans == 'where':
            ...
            'what? ',
            print
            '(expecting "y", "n", "why", "where", "browse", or "stop")'
...
def print_solns(facts, filter):
    ...
    if not sources['rule']:
        print
        'I have not deduced any new facts.'
    else:
        print
        'I deduced these facts...'
        for (fact, proof) in sources['rule']:
            print
            '  ', external([fact])  # , '(by rule', proof[0] + ')'
...
def trace_tree((fact, proof), level):
    ...
    if proof == 'told':
        ...
        print
        'was on your initial facts list'
    elif proof == 'atomic':
        print
        'is an absolute truth'
    elif proof == 'not':
        print
        'was a negative answer, or was ommitted'
    else:
        rule, subproof = proof
        print
        'was deduced by firing rule', rule
